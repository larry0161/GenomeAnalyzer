...
@ray.remote
def searchNCBI(data_split,file_num):
    temp_file = open('file'+str(file_num),'w')
    
    server = "http://grch37.rest.ensembl.org/variation/human/"
    endpoint = "?phenotypes=1"
    for i in data_split:
        rsid,chrome,position,alleles =i.split()

        r = requests.get(server+rsid+endpoint, headers={ "Content-Type" : "application/json"})
        gene = 'N/A'
        most_severe = 'N/A'
        detected = 'N/A'
        trait = 'N/A'
...
step = data_length//cpu
steps = [i for i in range(0,data_length-step+1,step)]
steps.append(data_length)

def go():
    ray.get([searchNCBI.remote(data[steps[i]:steps[i+1]],i) for i in range(cpu)])
go()
